...
\begin{tikzpicture}[node distance=0.35cm and 0.2cm, auto, >=stealth',
    ...
    % Symmetrically positioned candidate classifiers with 3.0cm spacing (no overlap)
    \node [smallbox] (lr)  at (-4.5, -1.85) {Logistic\\Regression};
    \node [smallbox] (svm) at (-1.5, -1.85) {Linear\\SVC / SVM};
    \node [smallbox] (rf)  at (1.5, -1.85) {Random\\Forest};
    \node [smallbox] (nb)  at (4.5, -1.85) {Multinomial\\Naive Bayes};
    
    \node [process, text width=13cm] (collect) at (0, -3.3) {Collect Predictions from All Classifiers};
    ...
    \path [arrow] (input.south) -- (0, -1.3) -| (lr.north);
    \path [arrow] (input.south) -- (0, -1.3) -| (svm.north);
    \path [arrow] (input.south) -- (0, -1.3) -| (rf.north);
    \path [arrow] (input.south) -- (0, -1.3) -| (nb.north);
