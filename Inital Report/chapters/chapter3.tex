...
\begin{tikzpicture}[node distance=0.35cm and 0.2cm, auto, >=stealth',
    ...
    smallbox/.style={rectangle, draw=black, fill=white, text width=5.6em, text centered, rounded corners=2pt, minimum height=2.2em, font=\scriptsize},
    branch/.style={rectangle, draw=black, fill=white, text width=7.5em, text centered, rounded corners=2pt, minimum height=1.5em, font=\scriptsize},
    arrow/.style={draw, -latex', semithick}]
    ...
    \path [arrow] (lr.south) -- ++(0,-0.3) -| ([xshift=-1.5cm]collect.north);
    \path [arrow] (svm.south) -- ++(0,-0.3) -| ([xshift=-0.5cm]collect.north);
    \path [arrow] (rf.south) -- ++(0,-0.3) -| ([xshift=0.5cm]collect.north);
    \path [arrow] (nb.south) -- ++(0,-0.3) -| ([xshift=1.5cm]collect.north);
...
The proposed Medical Specialty Classification system follows a structured pipeline that transforms raw clinical transcription narratives into reliable diagnostic specialty predictions using machine learning. The pipeline begins with clinical text data acquisition, followed by text preprocessing, TF-IDF feature extraction, machine learning model development and voting ensemble aggregation, and finally medical specialty prediction through a user-friendly Flask web application. Each stage plays an important role in ensuring accurate and reliable classification across all 40 medical specialties.

\begin{figure}[H]
\centering
\resizebox{\linewidth}{!}{
\begin{tikzpicture}[
    box/.style={rectangle, draw=black, thick, fill=white, rounded corners=3pt, text centered, inner sep=4pt, font=\small},
    titlebox/.style={rectangle, draw=black, thick, fill=white, rounded corners=2pt, font=\normalsize\bfseries, inner sep=4pt},
    arr/.style={-latex', thick}
]

    % ======================== CONTAINER 1: DATA ========================
    \draw [thick, rounded corners=5pt] (0, 8.4) rectangle (15.8, 11.6);
    \node at (7.9, 11.6) [titlebox] {DATA};
    
    \node [box, text width=2.4cm, minimum height=1.7cm] (d1) at (1.5, 9.9) {\textbf{Kaggle}\\\textbf{MTSamples}\\\textbf{Dataset}};
    \node [box, text width=2.5cm, minimum height=1.7cm] (d2) at (4.7, 9.9) {\textbf{Raw Clinical}\\\textbf{Transcriptions}\\(4,966 Reports)};
    \node [box, text width=2.6cm, minimum height=1.7cm] (d3) at (8.0, 9.9) {\textbf{Text}\\\textbf{Preprocessing}\\(Cleaning, Stopwords,\\Lemmatization)};
    \node [box, text width=2.6cm, minimum height=1.7cm] (d4) at (11.3, 9.9) {\textbf{Feature Extraction}\\\textbf{TF-IDF Vectorizer}\\(Unigrams + Bigrams,\\Sublinear TF)};
    \node [box, text width=2.4cm, minimum height=1.7cm] (d5) at (14.4, 9.9) {\textbf{Feature Dataset}\\\textbf{Sparse Matrix}\\(\texttt{features.csv})};
    ...
    % Routing from Container 1 to Container 2
    \draw [arr] (d5.south) -- ++(0,-0.6) -| (0.6, 7.8) -- (0.6, 5.8) -- (1.0, 5.8);

    % ======================== CONTAINER 2: MODEL BUILDING ========================
    \draw [thick, rounded corners=5pt] (0, 3.4) rectangle (15.8, 7.6);
    \node at (7.9, 7.6) [titlebox] {MODEL BUILDING};
    
    \node [box, text width=2.2cm, minimum height=2.4cm] (m1) at (2.2, 5.5) {\textbf{Train--Test}\\\textbf{Split}\\(80\% / 20\%)};
    
    \node [box, text width=2.8cm, minimum height=0.65cm] (m_svm) at (5.4, 6.7) {\textbf{Linear SVM}};
    \node [box, text width=2.8cm, minimum height=0.65cm] (m_rf) at (5.4, 5.9) {\textbf{Random Forest}};
    \node [box, text width=2.8cm, minimum height=0.65cm] (m_lr) at (5.4, 5.1) {\textbf{Logistic Regression}};
    \node [box, text width=2.8cm, minimum height=0.65cm] (m_nb) at (5.4, 4.3) {\textbf{Multinomial NB}};
    
    \node [box, text width=2.5cm, minimum height=2.4cm] (m_ens) at (8.5, 5.5) {\textbf{Voting Ensemble}\\\textbf{Classifier}\\(Soft Consensus\\Voting)};
    \node [box, text width=2.4cm, minimum height=2.4cm] (m_eval) at (11.5, 5.5) {\textbf{Model Evaluation}\\Accuracy, F1-Score,\\Precision, Recall};
    \node [box, text width=2.2cm, minimum height=2.4cm] (m_best) at (14.3, 5.5) {\textbf{Select \& Save}\\\textbf{Best Model}\\(\texttt{pipeline.joblib})};
    
    \path [arr] (m1.east) -- ++(0.3,0) |- (m_svm.west);
    \path [arr] (m1.east) -- ++(0.3,0) |- (m_rf.west);
    \path [arr] (m1.east) -- ++(0.3,0) |- (m_lr.west);
    \path [arr] (m1.east) -- ++(0.3,0) |- (m_nb.west);
    
    \path [arr] (m_svm.east) -| ++(0.3,-0.6) -- (m_ens.west |- m_svm.east);
    \path [arr] (m_rf.east) -- (m_ens.west |- m_rf.east);
    \path [arr] (m_lr.east) -- (m_ens.west |- m_lr.east);
    \path [arr] (m_nb.east) -| ++(0.3,0.6) -- (m_ens.west |- m_nb.east);
    
    \path [arr] (m_ens) -- (m_eval);
    \path [arr] (m_eval) -- (m_best);

    % Routing from Container 2 to Container 3
    \draw [arr] (m_best.south) -- ++(0,-0.6) -| (0.6, 2.8) -- (0.6, 0.9) -- (1.0, 0.9);

    % ======================== CONTAINER 3: DEPLOYMENT ========================
    \draw [thick, rounded corners=5pt] (0, -0.6) rectangle (15.8, 2.6);
    \node at (7.9, 2.6) [titlebox] {DEPLOYMENT};
    
    \node [box, text width=2.4cm, minimum height=1.7cm] (dep1) at (1.5, 0.9) {\textbf{Flask Web}\\\textbf{Application}\\(User-Friendly UI)};
    \node [box, text width=2.4cm, minimum height=1.7cm] (dep2) at (4.5, 0.9) {\textbf{Input Clinical}\\\textbf{Transcription}\\(Paste / Upload)};
    \node [box, text width=2.6cm, minimum height=1.7cm] (dep3) at (7.6, 0.9) {\textbf{Feature Extraction}\\\textbf{\& Vectorization}\\(Using Saved Pipeline)};
    \node [box, text width=2.4cm, minimum height=1.7cm] (dep4) at (10.7, 0.9) {\textbf{Specialty}\\\textbf{Prediction}\\(40 Specialties)};
    \node [box, text width=2.6cm, minimum height=1.7cm] (dep5) at (13.8, 0.9) {\textbf{Assessment}\\\textbf{Report}\\(Prediction Result +\\Probability Insights)};
...
\begin{enumerate}
    ...
    \item \textbf{Machine Learning Model Development}: Multiple supervised machine learning algorithms, including Linear Support Vector Machine (Linear SVM), Random Forest (RF), Logistic Regression (LR), and Multinomial Naive Bayes (MNB), are trained independently using the extracted features.
    \item \textbf{Hyperparameter Tuning}: Grid search cross-validation is performed to optimize critical hyperparameters, including regularisation strengths ($C, \lambda$), smoothing parameters ($\alpha$), and tree estimators ($B$).
    \item \textbf{Ensemble Model Formulation}: A Voting Ensemble Classifier is constructed using soft-voting consensus aggregation to synthesize probability distributions from all candidate models, mitigating individual estimator variance.
    \item \textbf{Model Evaluation and Comparison}: All trained candidate models and the Voting Ensemble are evaluated using diagnostic multi-class metrics: Accuracy, Precision, Recall (Sensitivity), and F1-Scores.
    \item \textbf{Deployment}: The optimal pipeline, comprising the fitted TF-IDF vectorizer and trained Voting Ensemble classifier, is serialized using Joblib and integrated into a lightweight Flask web application.
    \item \textbf{Clinical Text Input and Prediction}: Healthcare professionals enter or upload patient clinical notes through the web application. The text is preprocessed and vectorized using the deployed pipeline, and the model predicts the most appropriate medical specialty alongside classification confidence probabilities.
\end{enumerate}
